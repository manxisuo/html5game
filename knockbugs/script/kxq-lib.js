...
Bug.prototype.beyondCanvas = function() {
	...
	if (this.x + this.width /2 <= 0 || this.x - this.width /2 >= canvasW) {
		return true;
	}

	if (this.y + this.height /2 <= 0 || this.y - this.height /2 >= canvasH) {
		return true;
	}
...
Bug.prototype.render = function(brush) {
	 brush.image(this.image, this.x, this.y, this.width, this.height, this.getAngle());
...
Bug.prototype.turn = function(angle) {
	...
	var curAngle = this.getAngle() + angle;
...
Bug.prototype.getAngle = function() {
